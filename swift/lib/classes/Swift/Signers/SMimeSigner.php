...
class Swift_Signers_SMimeSigner implements Swift_Signers_BodySigner
{
    ...
    {
        $this->signCertificate = 'file://'.str_ireplace('\\', '/', realpath($certificate));

        if (null !== $privateKey) {
            if (is_array($privateKey)) {
                $this->signPrivateKey = $privateKey;
                $this->signPrivateKey[0] = 'file://'.str_ireplace('\\', '/', realpath($privateKey[0]));
            } else {
                $this->signPrivateKey = 'file://'.str_ireplace('\\', '/', realpath($privateKey));
            }
        }

        $this->signOptions = $signOptions;
        if (null !== $extraCerts) {
            $this->extraCerts = str_ireplace('\\', '/', realpath($extraCerts));
        }
    ...
    public function setEncryptCertificate($recipientCerts, $cipher = null)
    {
        if (is_array($recipientCerts)) {
            ...
            foreach ($recipientCerts as $cert) {
                $this->encryptCert[] = 'file://'.str_ireplace('\\', '/', realpath($cert));
            }
        } else {
            $this->encryptCert = 'file://'.str_ireplace('\\', '/', realpath($recipientCerts));
        }
